...
export const useDeviceAuth = (): DeviceAuthHook => {
  ...
  const prepareAuthData = useCallback((): string => {
    try {
      ...
      // Для повышения надежности чтения QR-кода используем максимально простой формат
      // Включаем только ИИН и пароль без разделителей для надежного считывания
      // Для QR-кода используем только простую строку без специальных символов
      return `${iin}${password}`;
    } catch (e) {
